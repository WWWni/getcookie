...
class ExampleController extends Controller {
  ...
  async dbOperation(args) {
    ...
    switch (paramsObj.action) {
      ...
      case "del":
        ...
        break;
      case "update":
        data.result = await service.storage.updateTestData(
          paramsObj.update_name,
          paramsObj.update_age
        );
        break;
      case "get":
  ...
  async sqlitedbOperation(args) {
    ...
    switch (paramsObj.action) {
      case "add":
        ...
        break;
      case "del":
        data.result = await service.storage.delTestDataSqlite(
          paramsObj.delete_name
        );
        break;
      case "update":
        data.result = await service.storage.updateTestDataSqlite(
          paramsObj.update_name,
          paramsObj.update_age
        );
        break;
      case "get":
        data.result = await service.storage.getTestDataSqlite(
          paramsObj.search_age
        );
        break;
    }
  ...
  createWindow(args) {
    ...
    const device = [
      ...
      "Mozilla/5.0 (iPad; CPU OS 12_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/12.1.2 Mobile/15E148 Safari/604.1",
      "Mozilla/5.0 (iPad; CPU OS 11_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/11.0 Mobile/15E148 Safari/604.1",
      "Mozilla/5.0 (Linux; Android 11; Pixel C Build/RQ2A.210305.006) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.152 Safari/537.36",
      "Mozilla/5.0 (Linux; Android 10; Pixel C Build/QQ2A.200305.003) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/80.0.3987.117 Safari/537.36",
      ...
      "Mozilla/5.0 (Linux; Android 10; SAMSUNG SM-A107F/A107FXXS8BUC1) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/13.2 Chrome/83.0.4103.106 Mobile Safari/537.36",
  ...
  getCookies() {
    return new Promise(async (resolve, reject) => {
      const ses = winObj.webContents.session;
      let a = await ses.cookies.get({}, (error, cookies) => {
